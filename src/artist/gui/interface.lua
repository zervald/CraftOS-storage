...
return function(context, extract_items)
  ...
  local item_list = ItemList({
    ...
    selected = function(item)
      ...
      local function extract()
        if input.value then
          extract_items(item.hash, input.value)
        end
        ui:pop()
      end
  ...
  context.mediator:subscribe("item_list.update", function(items)
    item_list:update_items(items)
  end)
